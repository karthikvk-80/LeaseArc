    ...
    *,
    ...
    schema = {
        ...
        "single_value": {
            ...
            "confidence_reason": "concise explanation for the confidence score",
            "source": {
    ...
    if anchor_context and anchor_context.strip():
        anchor_block = f"""
CONFIRMED PHASE 1 FACTS
{anchor_context.strip()}

ANCHOR FACT GUARDRAIL
Use CONFIRMED PHASE 1 FACTS only to resolve document order, current-vs-superseded terms,
parties, dates, and interpretation. Do not extract a Phase 2 value from anchor facts alone;
the final value still needs supporting source_clause from the provided profiler context.
Treat low-confidence anchor facts as tentative and verify them against the provided profiler context.
""".strip()
...
- If no current value is found, return is_enumerate=false with all field values null and a not_found trace.
	...
	For long clause abstractions or answers supported by multiple nearby passages, choose one primary
	contiguous source_clause that best supports the final answer for UI highlighting. Put any extra
	supporting context or correlation in trace entries instead of expanding source.source_clause.
	...
	Every trace item must include a concise reason explaining why that context made the value current,
	inferred, calculated, superseded, rejected, or not found. For calculated dates, mention the stated
	inputs used for the calculation. For inferred values, mention the context facts that support the
	inference.

	CONFIDENCE RULE
	Use confidence_score and confidence_level as your honest self-assessment of how well the provided
	context supports the final answer. Also return confidence_reason with a concise explanation of why
	the evidence warrants that specific score and level. Do not default to high confidence.
	- high / 80-100: the value is directly stated, or it is an obvious inference/calculation from clear
	  current context, with no material conflict or ambiguity.
	- medium / 50-79: the value is reasonably supported but depends on non-obvious inference, amendment
	  ordering, conditional language, OCR cleanup, or correlation across passages.
	- low / 1-49: the value is weakly supported, ambiguous, possibly superseded, OCR-noisy in a way that
	  affects meaning, or based on incomplete context.
	- 0: no value was found or all fields are null.
	If you infer or calculate a value, explain that basis in trace.reason. Clear, routine lease inferences
	such as building type from permitted use or premises description may still be high confidence when the
	context strongly supports them.
