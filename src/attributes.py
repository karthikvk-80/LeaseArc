...
PARTY_CONTACT_FIELDS = ["Value", "Role", *CONTACT_IDENTIFICATION_FIELDS]
...
REFERENCE_HINTS: dict[str, Any] = {
    "by_suffix": {
        ...
        "Building Type": {
            ...
            "format": "Short descriptor such as Office, Commercial office, Retail, Industrial, Warehouse, Mixed-use. If not explicitly labeled, infer from permitted use, tenant operations, premises description, or building context with medium confidence.",
        ...
        "Landlord Name": {
            "hint": "Full legal name, role, and available notice/contact details for each current landlord in the lease chain, including direct landlord/sub-lessor and superior/head/ground landlord when supported.",
            "look_in": "Parties clause, recitals, underlying/head lease references, definitions, notice clauses, signature block, amendments, assignment documents.",
            "format": "One item per landlord with full legal entity/person name, concise role, Contact Type, Name, and any supported address/contact fields.",
            "exclude": "Do not return tenant/licensee/sub-lessee name.",
        },
        "Tenant Name": {
            "hint": "Full legal name and available notice/contact details for each current tenant, lessee, licensee, sub-lessee, occupant, or assignee tenant.",
            "look_in": "Parties clause, recitals, notice clauses, signature block, amendments, assignment documents.",
            "format": "One item per tenant with full legal entity/person name, Contact Type, Name, and any supported address/contact fields.",
            "exclude": "Do not return landlord/licensor/sub-lessor name.",
        },
        "Contact Type": {
            "hint": "Party/contact role for this contact row.",
            "format": "Landlord, Tenant, Notice Contact, Billing Contact, Property Manager, Broker, or another stated role.",
            "exclude": "Do not invent a role that conflicts with the party clause.",
        },
        "Name": {
            "hint": "Legal entity/person/contact name for this row.",
            "format": "Name exactly as stated where possible.",
            "exclude": "Do not merge multiple entities into one name.",
        },
        "Attention": {"hint": "Attention line or person/department to whom notices are addressed.", "format": "Attention line only.", "exclude": "Do not use signatory name unless it is used as an attention/contact line."},
        "Care of": {"hint": "Care-of/c/o addressee detail.", "format": "Care-of party only.", "exclude": "Do not duplicate the main party name."},
        "DBA": {"hint": "Doing-business-as/trade name for this party.", "format": "DBA/trade name only.", "exclude": "Do not infer from property name."},
        "Street no.": {"hint": "Street/building number for this party/contact address.", "format": "Identifier only where separable.", "exclude": "Do not use property street number unless it is the contact address."},
        "Suite": {"hint": "Suite/unit/floor/office detail for this party/contact address.", "format": "Suite/unit/floor detail only.", "exclude": "Do not use leased premises suite unless the contact address says so."},
        "P.O. Box": {"hint": "P.O. Box or mailbox detail for this party/contact address.", "format": "P.O. Box value only.", "exclude": "Do not place ZIP/postal code here."},
        "Zip code": {"hint": "ZIP/PIN/postal code for this party/contact address.", "format": "Code only.", "exclude": "Do not use property postal code unless it is also this contact address."},
        "State / Province": {"hint": "State/province/region for this party/contact address.", "format": "State/province only.", "exclude": "Do not use incorporation jurisdiction unless it is the address state."},
        "Additional address details": {"hint": "Any remaining address details that do not fit street, suite, city, state, country, or postal fields.", "format": "Concise address fragment.", "exclude": "Do not duplicate fields already extracted separately."},
        "Phone": {"hint": "Telephone number for this party/contact.", "format": "Phone number exactly as stated.", "exclude": "Do not put fax or mobile here when labeled separately."},
        "Mobile": {"hint": "Mobile/cell number for this party/contact.", "format": "Mobile number exactly as stated.", "exclude": "Do not infer from unlabeled phone unless clearly mobile."},
        "Fax": {"hint": "Fax number for this party/contact.", "format": "Fax number exactly as stated.", "exclude": "Do not put phone number here unless labeled fax."},
        "Email": {"hint": "Email address for this party/contact.", "format": "Email address exactly as stated.", "exclude": "Do not invent domain or normalize names into emails."},
        "Effective Date": {
        ...
        "Current Expiration Date": {
            ...
            "look_in": "Latest amendment, renewal, term sheet, current term clause, base lease if no amendments.",
            "format": "Date in YYYY-MM-DD format when directly stated or calculable from a stated commencement date plus duration; otherwise preserve conditional language.",
        ...
        "Original Expiration Date": {
            ...
            "format": "Date in YYYY-MM-DD format when directly stated or calculable from original commencement plus duration. Calculate only when directly supported.",
        ...
        "Delivery Date": {
            ...
            "format": "Date in YYYY-MM-DD format when fixed; if delivery/handover is conditional and no fixed date is stated, return the conditional delivery language.",
            "exclude": "Return null only if no delivery or handover obligation is stated.",
        },
        "Term Duration": {
        ...
        "Lease Status": {
            "hint": "Current lease lifecycle state such as Active, Expired, Terminated, Surrendered, Renewed, MTM, Indefinite, Holdover, or Unknown.",
            ...
            "exclude": "Do not return Registered, Executed, Signed, Recorded, or similar document status unless the requested field explicitly means document status. Do not overstate active/expired if dates or documents do not support it.",
        },
        "Default": {
    ...
    "party_contact_group_note": "PARTY CONTACTS: for Landlord Name and Tenant Name, extract one item per distinct party. Fill Value and Name with the party/contact name, set Contact Type to Landlord or Tenant unless the text states a more specific contact role, and populate address/phone/email fields only when supported by the provided context. Leave missing contact fields null.",
...
def reference_guidance(name: str, category: str, expected_fields: list[str], repeatable_hint: bool) -> dict[str, Any]:
    ...
    if repeatable_hint:
        ...
        group_note_keys = {
            ...
            "Landlord Name": "party_contact_group_note",
            "Tenant Name": "party_contact_group_note",
        }
...
ATTRIBUTE_SPECS: list[AttributeSpec] = [
    spec("Property name", "Core Lease Terms", ["Extract the building, project, complex, mall, or property name for the leased premises.", "Look near recitals, definitions, schedules, premises descriptions, and property address blocks."]),
    spec("Street", "Core Lease Terms", ["Extract the street, road, block, complex road, or address line excluding city/state/country when possible.", "Use premises address, demised premises, schedule of land/building, and notice address only when it identifies the property."]),
    spec("Street no", "Core Lease Terms", ["Extract the street number, plot number, CTS number, survey number, building number, or equivalent property identifier.", "Prefer property identifiers in the premises or land description over party notice addresses."]),
    spec("Postal code", "Core Lease Terms", ["Extract ZIP/PIN/postal code for the leased property address.", "Use property schedule or premises address, not landlord or tenant mailing address unless it is clearly the property."]),
    spec("City", "Core Lease Terms", ["Extract the city or municipality of the leased property.", "Look for city names in the property address, schedule, and premises description."]),
    spec("County", "Core Lease Terms", ["Extract county, district, taluka, village, or local administrative area for the property.", "For India leases, taluka, district, village, or registration area may satisfy this field."]),
    spec("State / province", "Core Lease Terms", ["Extract state, province, or region of the leased property.", "Use the property location, not party incorporation addresses."]),
    spec("Country", "Core Lease Terms", ["Extract the country where the leased property is located.", "Infer only when the property address, governing jurisdiction, registration data, or country metadata clearly supports it."]),
    spec("Building Type", "Core Lease Terms", ["Extract the asset/building type such as office, retail, commercial building, mall, warehouse, or mixed-use.", "Look in schedules, building description, use clauses, and premises description."]),
    spec("Total building area", "Core Lease Terms", ["Extract total building area if stated and preserve the source's area basis.", "Distinguish built-up, usable, leasable, rentable, and carpet area rather than treating them as interchangeable."]),
    spec("UOM", "Core Lease Terms", ["Extract unit of measure for the area value, such as sq ft, square feet, sq m, acres, or square meters.", "Use the unit tied to Total building area or the closest relevant area field."]),
    spec(
        "Landlord Name",
        "Parties",
        [
        ...
        expected_fields=PARTY_CONTACT_FIELDS,
    ...
    spec(
        ...
        "Parties",
        [
            "Extract tenant, lessee, licensee, sub-lessee, occupant, or transferee name and any available notice/contact/address details.",
            "If multiple current tenants or tenant contacts are present, enumerate them separately.",
            "Resolve amendments, assignments, and name changes to the current party while preserving history in trace.",
        ],
        expected_fields=PARTY_CONTACT_FIELDS,
        repeatable_hint=True,
    ),
    spec("Effective Date", "Critical Dates", ["Extract the effective date of the underlying lease.", "An amendment's document date belongs in trace unless it expressly changes the lease effective date."]),
    spec("Execution Date", "Critical Dates", ["Extract signing/execution date.", "Look near signature blocks, executed on, signed on, dated this, registration, and amendment letter dates."]),
    spec("Original Commencement Date", "Critical Dates", ["Extract original lease commencement/start date from the base lease.", "Do not replace this with amendment renewal dates unless the original date itself is restated."]),
    spec(
        "Rent Commencement Date",
        "Critical Dates",
        [
    ...
    spec("Current Commencement Date", "Critical Dates", ["Extract current operative commencement/start date after amendments or renewals.", "Prefer later amendment/renewal terms when they replace the base lease term."]),
    spec("Current Expiration Date", "Critical Dates", ["Extract current operative expiration/expiry/end date after amendments or renewals.", "Use a later document only when it expressly changes the term or clearly states the lease expiry."]),
    spec("Original Expiration Date", "Critical Dates", ["Extract original lease expiration/end date from the base lease.", "Do not replace this with renewed expiration unless the base expiration is restated as original."]),
    spec("Possession Date", "Critical Dates", ["Extract possession, handover, delivery of possession, or premises access date.", "Look in commencement, delivery, possession, fit-out, and handover clauses."]),
    spec("Delivery Date", "Critical Dates", ["Extract landlord delivery/handover date for premises or possession.", "Use only if a delivery or handover obligation/date is actually stated."]),
    spec("Term Duration", "Critical Dates", ["Extract lease term length/duration, such as months, years, lock-in period, renewal term, or current term.", "Use current governing term where amendments supersede base terms."]),
    spec(
        "Lease Status",
        "Restrictive Clauses",
        [
            "Extract current lifecycle status such as Active, Expired, Terminated, Surrendered, Renewed, MTM, Indefinite, Holdover, or Unknown.",
            "Do not return document status values such as Registered, Executed, Signed, or Recorded.",
            "Use amendment/surrender/termination documents and term dates to determine current state when available.",
        ],
    ),
    spec("Lease_Abstraction.Default", "Restrictive Clauses", ["Extract monetary and non-monetary default events, notices, cure periods, default interest, remedies, and default-based termination rights.", "Do not use an ordinary termination-for-convenience provision as the primary default result.", "Resolve amendments that change default remedies or notice/cure periods."]),
    spec("Lease_Abstraction.Estoppel", "Restrictive Clauses", ["Extract estoppel certificate obligations, delivery timelines, requested certifications, and party requirements.", "Look for estoppel, certificate, confirmation, or statement clauses."]),
    spec("Lease_Abstraction.Business Hours", "Restrictive Clauses", ["Extract operating or access hours for the building, premises, services, HVAC, or business operations.", "Look for business hours, working hours, normal hours, operating hours, and after-hours services."]),
    spec("Lease_Abstraction.Late Charges", "Restrictive Clauses", ["Extract late fee, interest on delayed payment, penalty, default interest, or overdue charge provisions.", "Include rate, grace period, trigger, and compounding/payment basis if stated."]),
    spec("Lease_Abstraction.Repair and Maintenance", "Restrictive Clauses", ["Extract landlord and tenant repair, maintenance, service-level, building upkeep, and premises upkeep obligations.", "Separate current obligations from superseded base terms in trace."]),
    spec("Lease_Abstraction.Insurance Requirements", "Restrictive Clauses", ["Extract insurance obligations, policy types, coverage requirements, insured parties, and indemnity-linked insurance duties.", "Look for insurance, policy, coverage, public liability, fire, property, and casualty language."]),
    spec("Lease_Abstraction.Parking", "Restrictive Clauses", ["Extract parking rights, number of spaces, location, charges, reserved/open parking, and access rules.", "Look in grant clauses, schedules, premises description, and parking tables."]),
    spec("Lease_Abstraction.Signage", "Restrictive Clauses", ["Extract signage rights, restrictions, approvals, locations, building directory, exterior/interior signs, and removal duties.", "Look for signage, signboard, name board, branding, facade, and directory clauses."]),
    spec("Lease_Abstraction.Surrender", "Restrictive Clauses", ["Extract the generally applicable surrender obligations at expiry or termination, including restoration, yield-up, handback, removal, timing, and condition.", "Include materially different partial surrender or special termination scenarios without replacing the general rule."]),
    spec("Lease_Abstraction.Holdover", "Restrictive Clauses", ["Extract holdover/overstay terms, rent multiplier, damages, month-to-month status, and landlord remedies.", "Look for holding over, overstay, unauthorized occupation, and continued possession."]),
    spec("Lease_Abstraction.Permitted Use", "Restrictive Clauses", ["Extract the actual business activities and occupancy uses expressly allowed in the premises.", "Use generic legal-compliance conditions as qualifications, not as a substitute for the permitted activities.", "Look for use clause, permitted use, business purpose, office use, commercial use, and restrictions tied to use."], profiler_aliases=["Lease_Abstraction.Permitted_Use"]),
    spec("Lease_Abstraction.Assignment/Sublet", "Restrictive Clauses", ["Extract assignment, subletting, transfer, sharing, group company occupancy, and consent requirements.", "Resolve amendments that add or relax assignment/sublease rights."]),
    spec("Lease_Abstraction.Alterations", "Restrictive Clauses", ["Extract alteration, improvement, fit-out, construction, approval, removal, and restoration rules.", "Look for tenant works, additions, modifications, structural changes, and fit-out clauses."]),
    spec("Lease_Abstraction.Operating Expenses", "Restrictive Clauses", ["Extract operating expense, CAM, maintenance charge, common area charge, service charge, and reimbursement provisions.", "Include exclusions, caps, audit rights, and payment frequency if stated."]),
    spec("Lease_Abstraction.RE Taxes", "Restrictive Clauses", ["Extract real estate/property tax obligations, increases, reimbursements, assessments, and tax pass-throughs.", "Look for property tax, municipal tax, rates, cess, assessment, and government levies."]),
    spec("Lease_Abstraction.Property Insurance", "Restrictive Clauses", ["Extract property insurance obligations, insured property, casualty coverage, premium reimbursements, and landlord/tenant responsibilities.", "Distinguish from general liability insurance if possible."]),
    spec("Lease_Abstraction.Restricted Uses", "Restrictive Clauses", ["Extract uses that are restricted, conditioned, or require consent.", "Look for restricted use, limitations on operations, rules, building regulations, and prohibited conduct."]),
    spec("Lease_Abstraction.Prohibited Uses", "Restrictive Clauses", ["Extract expressly prohibited uses, activities, hazardous uses, nuisance, illegal activity, or competing use restrictions.", "Return the current governing prohibition language."]),
    spec("Lease_Abstraction.Exclusive Use", "Restrictive Clauses", ["Extract exclusive use rights, exclusivity protection, prohibited competing tenants, and carve-outs.", "If no exclusivity is granted, return null with not_found trace."]),
    spec("Lease_Abstraction.Percentage Rent (Payment)", "Restrictive Clauses", ["Extract percentage rent payment obligations, sales thresholds, breakpoint, percentage rate, and payment timing.", "Use only if percentage rent is actually stated."]),
    spec("Lease_Abstraction.Gross Sales (Reporting)", "Restrictive Clauses", ["Extract gross sales reporting obligations, statements, audit rights, reporting frequency, and excluded sales.", "Use only if sales reporting is actually stated."]),
    spec("Lease_Abstraction.Go dark", "Restrictive Clauses", ["Extract go-dark, continuous operation, closure, vacancy, or non-operation obligations and remedies.", "Look for continuous operation, cease operations, remain open, or abandonment language."]),
    spec("Lease_Abstraction.Co-Tenancy", "Restrictive Clauses", ["Extract co-tenancy requirements, anchor tenant conditions, remedies, rent relief, or termination rights.", "Use only if co-tenancy language is actually stated."]),
    spec("Lease_Abstraction.Radius Restrictions", "Restrictive Clauses", ["Extract radius restriction, competing location, restricted area, distance limits, and exceptions.", "Use only if radius or competing store restrictions are actually stated."]),
    spec("Lease_Abstraction.Tenant Improvement Allowance", "Restrictive Clauses", ["Extract tenant improvement allowance, fit-out allowance, reimbursement, payment conditions, deadlines, and unused allowance treatment.", "Look for allowance, tenant improvements, fit-out works, and landlord contribution."]),
    spec("Lease_Abstraction.Brokers", "Restrictive Clauses", ["Extract broker, commission, brokerage representation, indemnity, and payment obligations.", "Use only if brokerage language is actually stated."]),
    spec("Lease_Abstraction.Notices", "Restrictive Clauses", ["Extract notice requirements, notice addresses, delivery methods, deemed delivery, attention lines, and update procedure.", "Resolve later amendment notice address changes as current."]),
    spec("Lease_Abstraction.Base Rent Comments", "Restrictive Clauses", ["Extract narrative base rent comments, rent concessions, escalations, abatements, amendments, or superseding rent notes.", "Use latest amendment terms where they modify rent."]),
    spec("Lease_Abstraction.Utilities", "Restrictive Clauses", ["Extract utility obligations for electricity, power backup, water, sewer, HVAC, metering, billing, and service interruptions.", "Look for utilities, services, raw power, backup power, water, sewerage, and consumption charges."]),
    spec(
        "Area",
        "Area",
