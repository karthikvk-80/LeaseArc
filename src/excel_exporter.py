...
def _poc_rows(payload: dict[str, Any]) -> list[list[Any]]:
    ...
    for item in payload.get("attributes_flattened", []):
        if (
            item.get("group") in {"Landlord Name", "Tenant Name"}
            and item.get("sub_field") not in PARTY_OUTPUT_FIELDS
        ):
            continue
        rows.append(
            [
                ...
                _excel_value(item.get("source_file_name")),
                _excel_value(item.get("source_document_type")),
